...
class Game:
    ...
    #  Load level + reset toàn bộ stats 
    def load_level(self):
    ...
    def update_timer(self):
        """Gọi mỗi frame để cập nhật elapsed"""
        if self.timer_on:
            self.elapsed = time.time() - self.start_time
    ...
    def _render_hud(self):
        ...
        pad_x, pad_y = 10, SOKOBAN.WINDOW_HEIGHT - 148
        bg_rect = pygame.Rect(pad_x - 5, pad_y - 5, 280, 143)
        pygame.draw.rect(self.window, (240, 240, 240), bg_rect)
        pygame.draw.rect(self.window, SOKOBAN.BLACK,   bg_rect, 1)

        # Level
        self._hud_text(f"Level : {self.index_level} / 18",
                       pad_x, pad_y)
        # Method
        method_disp = self.current_method.upper() if self.current_method else "MANUAL"
        self._hud_text(f"Mode  : {method_disp}",
                       pad_x, pad_y + 18)
        # Steps hiện tại
        self._hud_text(f"Steps : {self.steps}",
                       pad_x, pad_y + 36)
        # Time hiện tại
        self._hud_text(f"Time  : {self.elapsed:.1f} s",
                       pad_x, pad_y + 54)

        #  Best score của level này
        ...
        self._hud_text(f"Best  : {bs_str} steps / {bt_str}",
                       pad_x, pad_y + 72, color=(0, 100, 180))
        ...
        self._hud_text(f"Status: {status_text}", pad_x, pad_y + 90, color=color)
        ...
        self.window.blit(
            font11.render("Score saved automatically on level complete",
                          True, SOKOBAN.GREY),
            (pad_x, pad_y + 124)
        )
    ...
    def auto_move(self, method='dfs'):
        ...
        try:
            strategy = get_move(self.level.structure[:-1],
        ...
        elapsed_solve = time.time() - t0

       
        if elapsed_solve >= TIMEOUT:
            
            self.solve_status = "TIMEOUT"
            print(f"[{method.upper()}] TIMEOUT ({elapsed_solve:.1f}s) - "
                  f"Nhấn N để sang level tiếp theo")
            return
        ...
        self.start_timer()
        print(f"[{method.upper()}] Solved in {len(strategy)} steps "
              f"({elapsed_solve:.2f}s solve time)")

        # Ghi ra file
        # with open("assets/sokobanSolver/Solverlevel_"
        #           + str(self.index_level) + ".txt", 'w+') as solver_file:
